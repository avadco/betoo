...
version: 1.0.0
description: |
  MANUAL TRIGGER ONLY: invoke only when user types /avad:document-release.
...
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./..avad/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not avad-specific):
...
```bash
gh pr view --json body -q .body > /tmp/avad-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/avad-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/avad-pr-body-$$.md
```
