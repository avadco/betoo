...
Shared reference for fetching, filtering, and classifying Greptile review comments on GitHub PRs. Both `/avad:review` (Step 2.5) and `/avad:ship` (Step 3.75) reference this document.
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ${CLAUDE_PLUGIN_ROOT}/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/..avad/projects/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior avad reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior avad reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains avad markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain avad markers.

3. **If a prior avad reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior avad reply exists:** Use Tier 1 (friendly) templates.
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ${CLAUDE_PLUGIN_ROOT}/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/..avad/projects/$REMOTE_SLUG"
mkdir -p ~/.avad
```
...
Example entries:
```
2026-03-13 | user/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | user/myapp | fix | app/models/user.rb | null-check
2026-03-13 | user/myapp | already-fixed | lib/payments.rb | error-handling
```
