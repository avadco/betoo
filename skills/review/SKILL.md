...
version: 1.0.0
description: |
  MANUAL TRIGGER ONLY: invoke only when user types /avad:review.
...
echo '{"skill":"review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
You are running the `/avad:review` workflow. Analyze the current branch's diff against the base branch for structural issues that tests don't catch.
...
1. Read `TODOS.md` (if it exists). Read PR description (`gh pr view --json body --jq .body 2>/dev/null || true`).
   ...
   **If no PR exists:** rely on commit messages and TODOS.md for stated intent — this is the common case since /avad:review runs before /avad:ship creates the PR.
...
2. If the doc file was NOT updated in this branch but the code it describes WAS changed, flag it as an INFORMATIONAL finding:
   "Documentation may be stale: [file] describes [feature/component] but code changed in this branch. Consider running `/avad:document-release`."

This is informational only — never critical. The fix action is `/avad:document-release`.
...
After all review passes complete, persist the final `/avad:review` outcome so `/avad:ship` can
recognize that Eng Review was run on this branch.

Run:

```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"COMMIT"}'
```
