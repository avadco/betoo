/**
 * avad CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read ..avad/avad:browse.json for port + token
...
export function resolveServerScript(
  ...
  // Compiled binary: derive the source tree from browse/dist/avad:browse
  if (execPath) {
...
export function resolveNodeServerScript(
  ...
  // Compiled binary: browse/dist/avad:browse → browse/dist/server-node.mjs
  if (execPath) {
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and avad
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`avad browse — Fast headless browser for AI coding agents
