...
version: 1.0.0
description: |
  MANUAL TRIGGER ONLY: invoke only when user types /avad:codex.
...
echo '{"skill":"codex","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
# /avad:codex — Multi-AI Second Opinion

You are running the `/avad:codex` skill. This wraps the OpenAI Codex CLI to get an independent,
...
"Codex CLI not found. Install it: `npm install -g @openai/avad:codex` or see https://github.com/openai/avad:codex"
...
1. `/avad:codex review` or `/avad:codex review <instructions>` — **Review mode** (Step 2A)
2. `/avad:codex challenge` or `/avad:codex challenge <focus>` — **Challenge mode** (Step 2B)
3. `/avad:codex` with no arguments — **Auto-detect:**
...
4. `/avad:codex <anything else>` — **Consult mode** (Step 2C), where the remaining text is the prompt
...
(e.g., `/avad:codex review focus on security`), pass them as the prompt argument:
```bash
...
6. **Cross-model comparison:** If `/avad:review` (Claude's own review) was already run
...
CROSS-MODEL ANALYSIS:
  ...
  Only Claude found: [findings unique to Claude's /avad:review]
  Agreement rate: X% (N/M total unique findings overlap)
```

7. Persist the review result:
```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N}'
```
...
(e.g., `/avad:codex challenge security`), include it:
...
or if plan files exist and the user said `/avad:codex` with no arguments:
```bash
ls -t ~/.claude/plans/*.md 2>/dev/null | xargs grep -l "$(basename $(pwd))" 2>/dev/null | head -1
```
If no project-scoped match, fall back to `ls -t ~/.claude/plans/*.md 2>/dev/null | head -1`
...
════════════════════════════════════════════════════════════
Tokens: N | Est. cost: ~$X.XX
Session saved — run /avad:codex again to continue this conversation.
```
...
agentic coding model). This means as OpenAI ships newer models, /avad:codex automatically
...
If the user specifies a model (e.g., `/avad:codex review -m gpt-5.1-codex-max`
or `/avad:codex challenge -m gpt-5.2`), pass the `-m` flag through to codex.
...
- **No double-reviewing.** If the user already ran `/avad:review`, Codex provides a second
