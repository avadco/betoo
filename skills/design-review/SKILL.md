...
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA}/analytics/skill-usage.jsonl 2>/dev/null || true
...
```
Write to: `${CLAUDE_PLUGIN_DATA}/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
