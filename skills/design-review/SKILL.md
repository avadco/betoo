...
version: 2.0.0
description: |
  MANUAL TRIGGER ONLY: invoke only when user types /avad:design-review.
...
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
REPORT_DIR="..avad/design-reports"
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Want to import cookies from your browser? Run `/avad:setup-browser-cookies` first if needed."
...
**Local:** `..avad/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/avad-slug 2>/dev/null)" && mkdir -p ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG
```
Write to: `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
Use the same scorecard format as /avad:plan-design-review (shown above). Fill in from both outputs.
