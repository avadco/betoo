...
source <(${CLAUDE_PLUGIN_ROOT}/bin/avad-repo-mode 2>/dev/null) || true
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
> Help avad get better! Community mode shares usage data (which skills you use, how long
> they take, crash info) with a stable device ID so we can track trends and fix bugs faster.
> No code, file paths, or repo names are ever sent.
> Change anytime with `avad-config set telemetry off`.

Options:
- A) Help avad get better! (recommended)
- B) No thanks

If A: run `${CLAUDE_PLUGIN_ROOT}/bin/avad-config set telemetry community`

If B: ask a follow-up AskUserQuestion:

> How about anonymous mode? We just learn that *someone* used avad — no unique ID,
...
If B→A: run `${CLAUDE_PLUGIN_ROOT}/bin/avad-config set telemetry anonymous`
If B→B: run `${CLAUDE_PLUGIN_ROOT}/bin/avad-config set telemetry off`
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+avad. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.

**Effort reference** — always show both scales:

| Task type | Human team | CC+avad | Compression |
|-----------|-----------|-----------|-------------|
...
Before building anything unfamiliar, **search first.** See `${CLAUDE_PLUGIN_ROOT}/ETHOS.md`.
...
If `_CONTRIB` is `true`: you are in **contributor mode**. At the end of each major workflow step, rate your avad experience 0-10. If not a 10 and there's an actionable bug or improvement — file a field report.
...
1. Check if the plan file already has a `## AVAD REVIEW REPORT` section.
2. If it DOES — skip (a review skill already wrote a richer report).
3. If it does NOT — run this command:

\`\`\`bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-read
\`\`\`

Then write a `## AVAD REVIEW REPORT` section to the end of the plan file:
...
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/avad:plan-ceo-review\` | Scope & strategy | 0 | — | — |
| Codex Review | \`/avad:codex review\` | Independent 2nd opinion | 0 | — | — |
| Eng Review | \`/avad:plan-eng-review\` | Architecture & tests (required) | 0 | — | — |
| Design Review | \`/avad:plan-design-review\` | UI/UX gaps | 0 | — | — |

**VERDICT:** NO REVIEWS YET — run \`/avad:autoplan\` for full review pipeline, or individual reviews above.
\`\`\`
...
* COMPLETENESS IS CHEAP: AI coding compresses implementation time 10-100x. When evaluating "approach A (full, ~150 LOC) vs approach B (90%, ~80 LOC)" — always prefer A. The 70-line delta costs seconds with CC. "Ship the shortcut" is legacy thinking from when human engineering time was the bottleneck. Go all the way.
...
```
If a design doc exists (from `/avad:office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and $BRANCH from the design doc check above):
```bash
HANDOFF=$(ls -t ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
from a prior CEO review session that paused so the user could run `/avad:office-hours`. Use it
...
> "No design doc found for this branch. `/avad:office-hours` produces a structured problem
> statement, premise challenge, and explored alternatives — it gives this review much
> sharper input to work with. Takes about 10 minutes. The design doc is per-feature,
> not per-product — it captures the thinking behind this specific change."

Options:
- A) Run /avad:office-hours now (we'll pick up the review right after)
- B) Skip — proceed with standard review

If they skip: "No worries — standard review. If you ever want sharper input, try
/avad:office-hours first next time." Then proceed normally. Do not re-offer later in the session.

If they choose A:

Say: "Running /avad:office-hours inline. Once the design doc is ready, I'll pick up
the review right where we left off."

Read the office-hours skill file from disk using the Read tool:
`${CLAUDE_PLUGIN_ROOT}/office-hours/SKILL.md`
...
"Could not load /avad:office-hours — proceeding with standard review."

After /avad:office-hours completes, re-run the design doc check:
```bash
SLUG=$(${CLAUDE_PLUGIN_ROOT}/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
...
sure," or is clearly exploring rather than reviewing — offer `/avad:office-hours`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /avad:office-hours is designed for. Want to run /avad:office-hours right now?
> We'll pick up right where we left off."

Options: A) Yes, run /avad:office-hours now. B) No, keep going.
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/avad-slug 2>/dev/null)" && mkdir -p ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/ceo-plans/{old-plan}.md ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/ceo-plans/archive/
```

Write to `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by /avad:plan-ceo-review on {date}
...
```bash
mkdir -p ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + avad,
...
The CEO calling in the designer. Not a pixel-level audit — that's /avad:plan-design-review and /avad:design-review. This is ensuring the plan has design intentionality.
...
If this plan has significant UI scope, recommend: "Consider running /avad:plan-design-review for a deep design review of this plan before implementation."
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
**Persist the result:**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
After implementation, run `/avad:design-review` on the live site to catch visual issues that can only be evaluated with rendered output.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+avad: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/avad-slug 2>/dev/null)"
rm -f ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/` (user config directory, not project files). The skill preamble
already writes to `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/sessions/` and `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /avad:ship.

```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`avad-config set skip_eng_review true\` (the "don't bother me" setting).
- **CEO Review (optional):** Use your judgment. Recommend it for big product/business changes, new user-facing features, or scope decisions. Skip for bug fixes, refactors, infra, and cleanup.
- **Design Review (optional):** Use your judgment. Recommend it for UI/UX changes. Skip for backend-only, infra, or prompt-only changes.
- **Adversarial Review (automatic):** Auto-scales by diff size. Small diffs (<50 lines) skip adversarial. Medium diffs (50–199) get cross-model adversarial. Large diffs (200+) get all 4 passes: Claude structured, Codex structured, Claude adversarial subagent, Codex adversarial. No configuration needed.
- **Outside Voice (optional):** Independent plan review from a different AI model. Offered after all review sections complete in /avad:plan-ceo-review and /avad:plan-eng-review. Falls back to Claude subagent if Codex is unavailable. Never gates shipping.
...
| Review | Trigger | Why | Runs | Status | Findings |
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/avad:plan-ceo-review\` | Scope & strategy | {runs} | {status} | {findings} |
| Codex Review | \`/avad:codex review\` | Independent 2nd opinion | {runs} | {status} | {findings} |
| Eng Review | \`/avad:plan-eng-review\` | Architecture & tests (required) | {runs} | {status} | {findings} |
| Design Review | \`/avad:plan-design-review\` | UI/UX gaps | {runs} | {status} | {findings} |
\`\`\`
...
- Search the plan file for a \`## AVAD REVIEW REPORT\` section **anywhere** in the file
  (not just at the end — content may have been added after it).
- If found, **replace it** entirely using the Edit tool. Match from \`## AVAD REVIEW REPORT\`
...
**Recommend /avad:plan-eng-review if eng review is not skipped globally** — check the dashboard output for `skip_eng_review`. If it is `true`, eng review is opted out — do not recommend it. Otherwise, eng review is the required shipping gate. If this CEO review expanded scope, changed architectural direction, or accepted scope expansions, emphasize that a fresh eng review is needed. If an eng review already exists in the dashboard but the commit hash shows it predates this CEO review, note that it may be stale and should be re-run.

**Recommend /avad:plan-design-review if UI scope was detected** — specifically if Section 11 (Design & UX Review) was NOT skipped, or if accepted scope expansions included UI-facing features. If an existing design review is stale (commit hash drift), note that. In SCOPE REDUCTION mode, skip this recommendation — design review is unlikely relevant for scope cuts.

**If both are needed, recommend eng review first** (required gate), then design review.

Use AskUserQuestion to present the next step. Include only applicable options:
- **A)** Run /avad:plan-eng-review next (required gate)
- **B)** Run /avad:plan-design-review next (only if UI scope detected)
...
- **B)** Keep in `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/` only (local, personal reference)
- **C)** Skip
