...
version: 1.0.0
description: |
  MANUAL TRIGGER ONLY: invoke only when user types /avad:ship.
...
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
You are running the `/avad:ship` workflow. This is a **non-interactive, fully automated** workflow. Do NOT ask for confirmation at any step. The user said `/avad:ship` which means DO IT. Run straight through and output the PR URL at the end.
...
1. **Check for a prior override on this branch:**
   ...
   grep '"skill":"ship-review-override"' ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_OVERRIDE"
   ```
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - Options: A) Ship anyway  B) Abort — run /avad:review or /avad:plan-eng-review first  C) Change is too small to need eng review
   - If CEO Review is missing, mention as informational ("CEO Review not run — recommended for product changes") but do NOT block
   - For Design Review: run `source <(${CLAUDE_PLUGIN_ROOT}/bin/avad-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /avad:design-review for a full visual audit post-implementation." Still never block.

3. **If the user chooses A or C,** persist the decision so future `/avad:ship` runs on this branch skip the gate:
   ```bash
   eval "$(${CLAUDE_PLUGIN_ROOT}/bin/avad-slug 2>/dev/null)"
   echo '{"skill":"ship-review-override","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","decision":"USER_CHOICE"}' >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/$SLUG/$BRANCH-reviews.jsonl
   ```
...
[ -f ..avad/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `..avad/no-test-bootstrap` and continue without tests.
...
If user picks C → write `..avad/no-test-bootstrap`. Tell user: "If you change your mind later, delete `..avad/no-test-bootstrap` and re-run." Continue without tests.
...
```bash
bin/test-lane 2>&1 | tee /tmp/avad:ship_tests.txt &
npm run test 2>&1 | tee /tmp/avad:ship_vitest.txt &
wait
```
...
- Switch to /avad:investigate mindset: root cause first, then minimal fix.
...
- Create a GitHub issue assigned to that person:
  ...
  gh issue create \
    ...
    --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** avad /avad:ship on <date>" \
...
`/avad:ship` is a pre-merge gate, so always use full tier (Sonnet structural + Opus persona judges).

```bash
EVAL_JUDGE_TIER=full EVAL_VERBOSE=1 bin/test-lane --eval test/evals/<suite>_eval_test.rb 2>&1 | tee /tmp/avad:ship_evals.txt
```
...
**Tier reference (for context — /avad:ship always uses `full`):**
| Tier | When | Speed (cached) | Cost |
|------|------|----------------|------|
| `fast` (Haiku) | Dev iteration, smoke tests | ~5s (14x faster) | ~$0.07/run |
| `standard` (Sonnet) | Default dev, `bin/test-lane --eval` | ~17s (4x faster) | ~$0.37/run |
| `full` (Opus persona) | **`/avad:ship` and pre-merge** | ~72s (baseline) | ~$1.27/run |
...
After producing the coverage diagram, write a test plan artifact so `/avad:qa` and `/avad:qa-only` can consume it:
...
Write to `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
Check if the diff touches frontend files using `avad-diff-scope`:

```bash
source <(${CLAUDE_PLUGIN_ROOT}/bin/avad-diff-scope <base> 2>/dev/null)
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
7. **After all fixes (auto + user-approved):**
   - If ANY fixes were applied: commit fixed files by name (`git add <fixed-files> && git commit -m "fix: pre-landing review fixes"`), then **STOP** and tell the user to run `/avad:ship` again to re-test.
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(${CLAUDE_PLUGIN_ROOT}/bin/avad-config get codex_reviews 2>/dev/null || true)
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
If Codex is not available for steps 1 and 3, note to the user: "Codex CLI not found — large-diff review ran Claude structured + Claude adversarial (2 of 4 passes). Install Codex for full 4-pass coverage: `npm install -g @openai/avad:codex`"

**Persist the review result AFTER all passes complete** (not after each sub-step):
```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
- Message: "avad recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
## Step 8.5: Auto-invoke /avad:document-release
...
1. Read the `/avad:document-release` skill: `cat ${CLAUDE_SKILL_DIR}/../document-release/SKILL.md`
...
doc updates — the user runs `/avad:ship` and documentation stays current without a separate command.
...
- **The goal is: user says `/avad:ship`, next thing they see is the review + PR URL + auto-synced docs.**
