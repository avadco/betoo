...
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA}/analytics/skill-usage.jsonl 2>/dev/null || true
...
1. **Check for a prior override on this branch:**
   ...
   grep '"skill":"ship-review-override"' ${CLAUDE_PLUGIN_DATA}/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_OVERRIDE"
   ```
...
3. **If the user chooses A or C,** persist the decision so future `/avad:ship` runs on this branch skip the gate:
   ```bash
   eval "$(${CLAUDE_PLUGIN_ROOT}/bin/avad-slug 2>/dev/null)"
   echo '{"skill":"ship-review-override","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","decision":"USER_CHOICE"}' >> ${CLAUDE_PLUGIN_DATA}/projects/$SLUG/$BRANCH-reviews.jsonl
   ```
...
Write to `${CLAUDE_PLUGIN_DATA}/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
