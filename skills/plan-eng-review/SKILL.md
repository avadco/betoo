...
version: 1.0.0
description: |
  MANUAL TRIGGER ONLY: invoke only when user types /avad:plan-eng-review.
...
echo '{"skill":"plan-eng-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/analytics/skill-usage.jsonl 2>/dev/null || true
...
After producing the coverage diagram, write a test plan artifact to the project directory so `/avad:qa` and `/avad:qa-only` can consume it as primary test input:
...
Write to `${CLAUDE_PLUGIN_DATA:-$HOME/.betoo}/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
This file is consumed by `/avad:qa` and `/avad:qa-only` as primary test input. Include only the information that helps a QA tester know **what to test and where** — not implementation details.
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/avad-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
...
**If no additional reviews are needed** (or `skip_eng_review` is `true` in the dashboard config, meaning this eng review was optional): state "All relevant reviews complete. Run /avad:ship when ready."
